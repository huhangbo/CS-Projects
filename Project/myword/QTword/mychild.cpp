...
bool MyChild::maybeSave()
{
    ...
    ret = QMessageBox::warning(this, tr("My Word"),tr("'%1'已经被改变,你想要保存吗?").arg(userFriendlyCurrentFile()),QMessageBox::Save | QMessageBox::Discard | QMessageBox::Cancel);
...
bool MyChild::saveAs()
{
    QString fileName = QFileDialog::getSaveFileName(this, tr("Save As"),curFile,tr("HTML file (*.htm *.html);;All files  (*.*)"));
    if (fileName.isEmpty())
        return false;
