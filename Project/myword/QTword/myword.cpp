...
{
    /*【文件】菜单动作集*/
    newAct = new QAction(QIcon(rsrcPath + "/filenew.png"), tr("新建"), this);
    newAct->setShortcuts(QKeySequence::New);
    newAct->setToolTip("新建");                   //设置工具栏按钮的提示文本
    newAct->setStatusTip(tr("新建一个文件"));     //设置状态栏提示文本
    connect(newAct, SIGNAL(triggered()), this, SLOT(fileNew()));

    openAct = new QAction(QIcon(rsrcPath + "/fileopen.png"), tr("打开"), this);
    openAct->setShortcuts(QKeySequence::Open);
    openAct->setToolTip("打开");
    openAct->setStatusTip(tr("打开已有文件"));
    connect(openAct, SIGNAL(triggered()), this, SLOT(fileOpen()));

    saveAct = new QAction(QIcon(rsrcPath + "/filesave.png"), tr("保存"), this);
    saveAct->setShortcuts(QKeySequence::Save);
    saveAct->setToolTip("保存");
    saveAct->setStatusTip(tr("保存此文件"));
    ...
    printAct = new QAction(QIcon(rsrcPath + "/fileprint.png"), tr("打印"), this);
    printAct->setShortcuts(QKeySequence::Print);
    printAct->setToolTip("打印");
    printAct->setStatusTip(tr("打印此文件"));
    connect(printAct, SIGNAL(triggered()), this, SLOT(filePrint()));

    printPreviewAct = new QAction(tr("打印预览"), this);
    printPreviewAct->setStatusTip(tr("打印预览"));
    ...
    /*【编辑】菜单动作集*/
    undoAct = new QAction(QIcon(rsrcPath + "/editundo.png"),tr("后退"), this);
    undoAct->setShortcut(QKeySequence::Undo);
    undoAct->setToolTip("后退");
    undoAct->setStatusTip(tr("撤销刚刚的操作"));
    connect(undoAct, SIGNAL(triggered()), this, SLOT(undo()));

    redoAct = new QAction(QIcon(rsrcPath + "/editredo.png"),tr("前进"), this);
    redoAct->setShortcut(QKeySequence::Redo);
    redoAct->setToolTip("前进");
    redoAct->setStatusTip(tr("重复之前的操作"));
    connect(redoAct, SIGNAL(triggered()), this, SLOT(redo()));

    cutAct = new QAction(QIcon(rsrcPath + "/editcut.png"),tr("剪贴"), this);
    cutAct->setShortcuts(QKeySequence::Cut);
    cutAct->setToolTip("剪贴");
    cutAct->setStatusTip(tr("将选中内容放到剪贴板"));
    connect(cutAct, SIGNAL(triggered()), this, SLOT(cut()));

    copyAct = new QAction(QIcon(rsrcPath + "/editcopy.png"),tr("复制"), this);
    copyAct->setShortcuts(QKeySequence::Copy);
    copyAct->setToolTip("复制");
    copyAct->setStatusTip(tr("将选中内容复制到剪贴板"));
    connect(copyAct, SIGNAL(triggered()), this, SLOT(copy()));

    pasteAct = new QAction(QIcon(rsrcPath + "/editpaste.png"),tr("粘贴"), this);
    pasteAct->setShortcuts(QKeySequence::Paste);
    pasteAct->setToolTip("粘贴");
    pasteAct->setStatusTip(tr("粘贴剪贴板中的内容"));
    connect(pasteAct, SIGNAL(triggered()), this, SLOT(paste()));

    CountAct =new QAction(tr("统计"),this);
    CountAct->setStatusTip(tr("统计文件中的字符数量"));
    connect(CountAct,SIGNAL(triggered()),this,SLOT(count()));

    CountT =new QAction(tr("特定统计"),this);
    CountT->setStatusTip(tr("统计特定字符"));
    connect(CountT,SIGNAL(triggered()),this,SLOT(countt()));

    FindAct =new QAction(tr("查找"),this);
    FindAct->setStatusTip(tr("查找文件中字符"));
    connect(FindAct,SIGNAL(triggered()),this,SLOT(find()));

    ReplaceAct = new QAction(QIcon(rsrcPath + "/find.png"),tr("替换"), this);
    ReplaceAct->setStatusTip(tr("替换文件中字符"));
    connect(ReplaceAct,SIGNAL(triggered()),this,SLOT(replace()));

    /*【格式】菜单动作集*/
    boldAct = new QAction(QIcon(rsrcPath + "/textbold.png"),tr("加粗"), this);
    boldAct->setCheckable(true);
    boldAct->setShortcut(Qt::CTRL + Qt::Key_B);
    boldAct->setToolTip("加粗");
    boldAct->setStatusTip(tr("加粗选中的字体"));
    QFont bold;
    bold.setBold(true);
    boldAct->setFont(bold);
    connect(boldAct, SIGNAL(triggered()), this, SLOT(textBold()));

    italicAct = new QAction(QIcon(rsrcPath + "/textitalic.png"),tr("斜体"), this);
    italicAct->setCheckable(true);
    italicAct->setShortcut(Qt::CTRL + Qt::Key_I);
    italicAct->setToolTip("斜体");
    italicAct->setStatusTip(tr("使选中字体倾斜"));
    QFont italic;
    italic.setItalic(true);
    italicAct->setFont(italic);
    connect(italicAct, SIGNAL(triggered()), this, SLOT(textItalic()));

    underlineAct = new QAction(QIcon(rsrcPath + "/textunder.png"),tr("下划线"), this);
    underlineAct->setCheckable(true);
    underlineAct->setShortcut(Qt::CTRL + Qt::Key_U);
    underlineAct->setToolTip("下划线");
    underlineAct->setStatusTip(tr("为所选字体添加下划线"));
    QFont underline;
    ...
    if (QApplication::isLeftToRight()) {
        leftAlignAct = new QAction(QIcon(rsrcPath + "/textleft.png"),tr("左对齐"), grp);
        centerAct = new QAction(QIcon(rsrcPath + "/textcenter.png"),tr("居中"), grp);
        rightAlignAct = new QAction(QIcon(rsrcPath + "/textright.png"),tr("右对齐"), grp);
    } else {
        rightAlignAct = new QAction(QIcon(rsrcPath + "/textright.png"),tr("右对齐"), grp);
        centerAct = new QAction(QIcon(rsrcPath + "/textcenter.png"),tr("居中"), grp);
        leftAlignAct = new QAction(QIcon(rsrcPath + "/textleft.png"),tr("左对齐"), grp);
    }
    justifyAct = new QAction(QIcon(rsrcPath + "/textjustify.png"),tr("两端对齐"), grp);

    leftAlignAct->setShortcut(Qt::CTRL + Qt::Key_L);
    leftAlignAct->setCheckable(true);
    leftAlignAct->setToolTip("左对齐");
    leftAlignAct->setStatusTip(tr("左对齐文档"));

    centerAct->setShortcut(Qt::CTRL + Qt::Key_E);
    centerAct->setCheckable(true);
    centerAct->setToolTip("居中");
    centerAct->setStatusTip(tr("居中显示"));

    rightAlignAct->setShortcut(Qt::CTRL + Qt::Key_R);
    rightAlignAct->setCheckable(true);
    rightAlignAct->setToolTip("右对齐");
    rightAlignAct->setStatusTip(tr("右对齐文档"));

    justifyAct->setShortcut(Qt::CTRL + Qt::Key_J);
    justifyAct->setCheckable(true);
    justifyAct->setToolTip("两端对齐");
    justifyAct->setStatusTip(tr("两端对齐"));

    QPixmap pix(16, 16);
    pix.fill(Qt::red);
    colorAct = new QAction(pix, tr("颜色"), this);
    colorAct->setToolTip("颜色");
    colorAct->setStatusTip(tr("为所选文本添加颜色"));
    ...
    closeAllAct = new QAction(tr("关闭所有"), this);
    ...
    nextAct->setStatusTip(tr("下一个窗口"));
    connect(nextAct, SIGNAL(triggered()),mdiArea, SLOT(activateNextSubWindow()));

    previousAct = new QAction(tr("上一个"), this);
    previousAct->setShortcuts(QKeySequence::PreviousChild);
    previousAct->setStatusTip(tr("上一个窗口"));
    ...
    aboutAct->setStatusTip(tr("关于此程序"));
...
void MyWord::createMenus()
{
    ...
    editMenu->addAction(CountT);
    editMenu->addAction(FindAct);
    editMenu->addAction(ReplaceAct);

    //【格式】主菜单
    formatMenu = menuBar()->addMenu(tr("格式"));
    fontMenu = formatMenu->addMenu(tr("字体"));   //【字体】子菜单
...
void MyWord::createToolBars()
{
    ...
    comboStyle->addItem("Number (Ⅰ.Ⅱ.Ⅲ.)");
    comboStyle->setStatusTip("添加符号或数字");
...
void MyWord::about()
{
    QMessageBox::about(this, tr("关于此程序"), tr("程序设计综合实践课程\n学号：17195476\n姓名：胡航博"));
}
...
void MyWord::count()
{
    ...
    if(activeMyChild())
    {
        ...
        QMessageBox::about(NULL, "统计", mesg);
    }
}
...
void MyWord::countt()
{
    ...
    if(activeMyChild())
    {
        ...
        bool ok;
        QString text=QInputDialog::getText(this,tr("统计特定字符串"),tr("请输入"),QLineEdit::Normal,nullptr,&ok);
        //将输入从String转换为char*
        ...
        if(ok && !text.isEmpty())
        {
            ...
            //message对话框
            QString mesg2=tr(" 特定串的数量为 %1\n ").arg(number);
            QMessageBox::about(NULL, "特定统计", mesg2);
        }
    }
...
void MyWord::find()
{
    ...
    new_FindDialog= new FindDialog;
    new_FindDialog->setPlainTextEdit((QPlainTextEdit*)activeMyChild());
    new_FindDialog->show();
...
void MyWord::replace()
{
    new_ReplaceDialog= new ReplaceDialog;
    new_ReplaceDialog->setPlainTextEdit((QPlainTextEdit*)activeMyChild());
    new_ReplaceDialog->show();
}
