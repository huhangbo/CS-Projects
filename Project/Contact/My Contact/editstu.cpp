...
void EditStu::ReadXml()
{
    ...
    while(!student.isNull())  //如果节点不空
    {
        if(student.isElement()) //如果节点是元素
        {
            ...
            {
                QDomNode factor=stuFactors.at(index);
                int i=0;
...
void EditStu::on_save_clicked()
{
    QString path=QFileDialog::getSaveFileName(this,tr("文件对话框"),"StudentInfo", tr("data(*.xml)"));
