...
{
    ui->setupUi(this);
    setWindowTitle(tr("挑 战 模 式"));
    {
        playlist->addMedia(QUrl("qrc:/res/main.mp3"));
        playlist->setPlaybackMode(QMediaPlaylist::Loop);
        music->setPlaylist(playlist);
        music->play();
    }
    {
        ui->showstep->setText(tr("30"));
        ui->showtime->setText(tr("01:00.0"));
...
void hard::mousePressEvent(QMouseEvent *event)
{
    int x = event->pos().x();
    int y = event->pos().y();
    if(20 < x && x < 400 && 20 < y && y < 400 && !end) // 100*4+20=420
    {
        int indX = (x - 20) / 100;
        int indY = (y - 20) / 100;
...
void hard::on_setpic_clicked()
{
    OpenFile = QFileDialog::getOpenFileName(this,
                                            "请选择照片",
                                            "D:\Qt\Project\Puzzle\Puzzle game\res\2",
...
void hard::on_set_step_clicked()
{
    bool ok;
    count = QInputDialog::getInt(this,tr("最大步数"),
                  tr("请输入"),30,0,100,1,&ok);
    ui->showstep->setText(tr("%1").arg(count));
}

void hard::on_set_time_clicked()
{
    bool ok;
    mins = QInputDialog::getInt(this,tr("最大分钟"),
                                tr("请输入"),1,0,10,1,&ok);
    ui->showtime->setText(tr("0%1:00.0").arg(mins));
}
